...
from scirnap import StringTie


class TestStringTie(unittest.TestCase):
    ...
    def test_stringtie(self):
        ...
        cu = StringTie('data/sorted_bams/',
                       'software/stringtie-2.1.4.OSX_x86_64/./stringtie',
                       ' -e ',
                       'stringtie/',
                       'software/ensembl/Homo_sapiens.GRCh38.100.gtf',
                       ctab_filepath='data/stringtie_ctab/',
                       dryrun=False, nthreads=5)
        files = cu.get_files_in_dir()
        # Run dryrun
        cu.run_per_file(files)
